...
if ( class_exists( 'WC_Email' ) ) :
	 ...
	 * @subpackage Auction_Software/woocommerce/classes/emails
	 */
	class WC_Auction_Software_Start extends WC_Email {
		...
		/**
		 * WC_Auction_Software_Start constructor.
		 */
		public function __construct() {

			global $woocommerce_auctions;

			$this->id          = 'auction_software_start';
			$this->title       = __( 'Auction Start', 'auction-software' );
			$this->description = __( 'Auction Start', 'auction-software' );

			$this->template_html  = 'emails/auction-software-start.php';
			$this->template_plain = 'emails/plain/auction-software-start.php';
			$this->template_base  = AUCTION_SOFTWARE_PLUGIN_TEMPLATE_PATH;

			$this->subject = __( '{product name} auction has started. Bid now!', 'auction-software' );
			$this->heading = __( 'Auction has started.', 'auction-software' );

			add_action( 'woocommerce_auction_software_start_notification', array( $this, 'trigger' ) );
		...
		public function trigger( $product_id ) {
			...
			if ( 0 === (int) $is_ended ) {
				...
				foreach ( $users as $user ) {
					$is_notified = WC_Auction_Software_Helper::check_if_user_notified_auction_start( $user->ID, $product_id );
					if ( 0 === (int) $is_notified ) {
						$emails[] = $user->user_email;
						update_post_meta( $product_id, 'auction_notify_' . $user->ID . '_' . $product_id . '_is_started', 1 );
					}
				}
				if ( ! empty( $emails ) ) {
			...
			} elseif ( 1 === (int) $is_ended ) {
				$users = get_users();
				foreach ( $users as $user ) {
					update_post_meta( $product_id, 'auction_notify_' . $user->ID . '_' . $product_id . '_is_started', 0 );
				}
			}
		}
	...
	return new WC_Auction_Software_Start();
endif;
