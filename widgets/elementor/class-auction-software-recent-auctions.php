...
/**
 * Elementor Widget_Recent_Auctions Widget.
 *
 * Elementor widget that inserts an embbedable content into the page, from any given URL.
 *
 * @since 1.0.0
 */
class Auction_Software_Recent_Auctions extends \Elementor\Widget_Base {

	/**
	 * Get widget name.
	 *
	 * Retrieve Widget_Recent_Auctions widget name.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 * Get widget title.
	 *
	 * Retrieve Widget_Recent_Auctions widget title.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function get_title() {
		return __( 'Auction Software Recent Auctions', 'auction-software' );
	}

	/**
	 * Get widget icon.
	 *
	 * Retrieve Widget_Recent_Auctions widget icon.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 *
	 * Retrieve the list of categories the Widget_Recent_Auctions widget belongs to.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	/**
	 * Register Widget_Recent_Auctions widget controls.
	 *
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'input_type'  => 'text',
				'default'     => __( 'Recent Auctions', 'auction-software' ),
				'placeholder' => __( 'Type your title here', 'auction-software' ),
			)
		);
		$this->add_control(
			'widget_post_no_recent',
			array(
	...
	/**
	 * Render Widget_Recent_Auctions widget output on the frontend.
	 *
	...
	protected function render() {
		...
		$title      = __( $settings['widget_title_recent'], 'auction-software' ); //phpcs:ignore
			$number = 5;
		if ( $settings['widget_post_no_recent'] ) {
			if ( ! is_numeric( $settings['widget_post_no_recent'] ) ) {
				$number = 10;
			} elseif ( $number < 1 ) {
				$number = 1;
			} elseif ( $number > 15 ) {
				...
				$number = $settings['widget_post_no_recent'];
			}
		}
		...
		if ( $r->have_posts() ) {
			$hide_time = empty( $instance['show_time_recent'] ) ? 0 : 1;
